...
// Transparent runs are written as fully transparent (0) so the region is fully
                                    ...
                                    const struct mp_bd_rle_elem *rle)
{
    ...
    for (int y = 0; y < h; y++) {
        uint32_t *out = dst + dst_stride * y;
        for (int x = 0; x < w;) {
            int len = in->len;
            if (len <= 0 || x + len > w)
                len = w - x; // guard against malformed runs
            uint32_t c = mp_bd_palette_to_bgra(&palette[in->color]);
            for (int i = 0; i < len; i++)
                out[i] = c;
            out += len;
            x += len;
